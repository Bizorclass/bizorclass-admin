...
export class HowToUseComponent {
  ...
  async getTermsAndConditionsDataFromServer() {
    try {
      this.isLoading = true;
      const getTermsAndConditionsDataResponse: any = await this.apiService.getHowToUseData();
      console.log("getTermsAndConditionsDataResponse => ", getTermsAndConditionsDataResponse);
      if (getTermsAndConditionsDataResponse?.data) {
        this.textData = getTermsAndConditionsDataResponse?.data?.how_to_use;
  ...
  async saveData() {
    try {
      this.isLoading = true;
      const updateCommonDataResponse: any = await this.apiService.updateCommonData({how_to_use: this.textData});
